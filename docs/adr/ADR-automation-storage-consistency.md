...
**Чесна межа цього висновку (critical-thinking C2, доданий FINAL
HARDENING циклом):** живим тестом перевірено ЛИШЕ per-read варіант
(`store.get(key, {consistency:"strong"})` у `getWithRetry`). Store-
level варіант (`getStore({name, consistency:"strong"})`) НЕ
перевірявся окремо — після одного production-збою й відкату не
повторювали ризик заради другого варіанту. Обидва методи документація
описує як такі, що потребують того самого `uncachedEdgeURL`
runtime-параметра — тому висновок "недоступно в цьому runtime"
поширюється на store-level з високою, але не 100% впевненістю
(інференція з природи помилки, не окремий вимір). Якщо колись
знадобиться остаточна відповідь — окремий, контрольований тест
store-level варіанту на некритичному endpoint, не інференція.
