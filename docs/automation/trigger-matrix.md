...
|---|---|---|---|---|---|
| Lead form submit (обидві форми) | Валідний email, не honeypot | `submission-created.js`: normalize+dedup email → upsert mock CRM contact, email-sink подія "lead_magnet", structured log | Немає (Netlify сам не ретраїть form hooks; ручний replay через `docs/runbooks/form-failure.md`) | Лише при винятку в самій функції (Netlify показує в Function logs — власник має дивитись вручну, автоматичного push-сповіщення немає, O-14 не закрито) | Redirect на `/thanks` (незалежно від успіху/невдачі mock CRM — форма НЕ повинна показувати помилку користувачу через внутрішній automation-збій) |
