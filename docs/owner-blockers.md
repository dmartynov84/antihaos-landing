...
## Нові докази цього циклу (DATA TRUTH & PAYMENT-SECURITY READINESS)

Жоден статус не змінився (усі 15 лишаються OPEN — жодного рішення
власника не надійшло цим циклом), але зібрано конкретніші докази:

- **O-03** (transactional storage): підтверджено остаточним живим
  тестом — `consistency:"strong"` недоступний в жодному з двох
  документованих варіантів API у поточному Lambda-compatibility
  runtime (не інференція, обидва варіанти дали ідентичну помилку).
  Це посилює, не змінює, статус O-03.
- **O-02** (payment provider): `docs/automation/ledger-contracts.md`/
  `payment-security-checklist.md` тепер явно фіксують, ЩО саме
  потрібно від вибору провайдера — explicit currency field,
  gross/fee/tax/net розділення, merchant ID перевірка. Не вигадано
  наперед, лише точно окреслено, що чекає на це рішення.
- **O-09** (data retention): Privacy тепер явно й чесно каже "строк
  ще не затверджений" (раніше — мовчала про це взагалі).
